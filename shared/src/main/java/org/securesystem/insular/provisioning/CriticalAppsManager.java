...
import org.securesystem.insular.engine.common.WellKnownPackages;
...
public class CriticalAppsManager {
	...
	@ProfileUser static Set<String> detectCriticalPackages(final PackageManager pm) {
		...
		final ApplicationInfo chrome_info = pm.getApplicationInfo(WellKnownPackages.PACKAGE_GOOGLE_CHROME, MATCH_UNINSTALLED_PACKAGES);
			pkgs.add(chrome_info.packageName);
		} catch (final PackageManager.NameNotFoundException ignored) {}
		return pkgs;
	}

	public static @Nullable String getCurrentWebViewPackageName() {
